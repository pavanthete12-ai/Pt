...
export default function Home(){
 ...
 useEffect(()=>{
  ...
  ws.onopen=()=>{
   setConnected(true);
   ws.send(JSON.stringify({type:"subscribe",symbol,timeframe:selectedTf}));
  };
  ...
  ws.onmessage=e=>{
   try{
    ...
    if(p.type==="candle"&&p.symbol===symbol){
     ...
     if(p.fusion)setFusion(p.fusion);
    }
   }catch{}
  };
  return()=>{
   try{ws.send(JSON.stringify({type:"unsubscribe",symbol,timeframe:selectedTf}))}catch{}
   ws.close();
   setConnected(false);
   setAnalysis(null);
   setFusion(null);
 ...
 return <main>
  ...
  <section className="hero"><div><p className="eyebrow">QUANTPULSE TERMINAL</p><h1>Multi-timeframe market intelligence in one terminal.</h1><p className="muted">Provider-fed OHLCV, technical analysis, multi-timeframe fusion and risk controls. Paper trading remains the default.</p></div><div className="gate"><button onClick={()=>setLive(!live)}>{live?"Disable live stream":"Connect live stream"}</button><small>Market-data streaming and real-money execution are separate controls.</small></div></section>
  ...
  <section className="cards"><div><b>{selectedTf} Trend</b><span>{tf?tf.trend+" • "+tf.signal+" • "+tf.confidence+"%":"Waiting for timeframe data"}</span></div><div><b>Pattern Scanner</b><span>{analysis?.pattern??"Waiting for candles"}</span></div><div><b>Momentum</b><span>{analysis?"RSI "+analysis.rsi+" • EMA9 "+analysis.ema9+" / EMA21 "+analysis.ema21:"Waiting for analysis"}</span></div><div><b>Paper Trading</b><span>Persistent orders + automatic SL/target monitoring</span></div></section>
  ...
  <footer>QuantPulse is analytical software. Signals are not guaranteed returns or profit probabilities. Real-money execution remains locked until broker authorization, security controls and risk validation are independently verified.</footer>
 </main>
}
